...
struct PhoneNumberInformationService {
    static let shared = PhoneNumberInformationService()

    func patchInformation(_ parameters: PhoneNumberRequest, delegate: SignUpPhoneVC) {
        ...
        AF.request(url, method: .patch, parameters: parameters, encoder: JSONParameterEncoder(), headers: header)
            .responseDecodable(of: InformationData.self, emptyResponseCodes: [200, 204, 205]) { response in
                print("폰 번호 정보 수정 response",response)
                switch response.result {
                case .success(let response):
                    delegate.didSuccessInformation(result:response)
                    print("폰 번호 정보 수정 성공햇니",response)
                case .failure(let error):
                    print("폰 번호 정보 수정에서 오류가 났습니다",error.localizedDescription)
